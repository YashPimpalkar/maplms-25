...
import DisplayMarks from "../../../components/DisplayMarks.jsx/DisplayMarks";
import TermworkAttainmentSaveToDatabase from "../../../components/CalculateAttainment/TermworkAttainmentSaveToDatabase";
import { FaUpload, FaBook, FaListAlt, FaSpinner, FaClipboardCheck } from "react-icons/fa";

const excludedCurriculumIds = [3,16];
const TheoryCurriculumIds = [1, 2];
const noCurriculumIds = [18];

const TermworkUpload = ({ uid }) => {
  const [curriculum, setCurriculum] = useState(null);
  const [termworkTable, setTermworkTable] = useState(null);
  const [termworkCurriculum, setTermworkCurriculum] = useState(null);
  ...
  useEffect(() => {
    if (userCourseId && curriculum && termworkTable) {
      ...
      if (selectedTermwork) {
        ...
        const termwork = filtered.filter(
          (item) =>
            !excludedCurriculumIds.includes(item.idcurriculum) &&
            !TheoryCurriculumIds.includes(item.idcurriculum) &&
            !noCurriculumIds.includes(item.idcurriculum)
        );

        setFilteredCurriculum(filtered);
        setTermworkCurriculum(termwork);
      }
    } else {
      setFilteredCurriculum(null);
    }
      ...
      <div className="max-w-screen-xl mx-auto mt-5">
        ...
        <div className="text-center mb-10">
          <div className="flex items-center justify-center mb-4">
            <FaUpload className="text-blue-600 text-5xl mr-3" />
            <h1 className="text-3xl md:text-4xl lg:text-5xl text-blue-700 font-bold">
              Assessment Upload
            </h1>
          </div>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Upload assessment marks for your courses. Select a course, choose an assessment type, and enter student marks.
          </p>
        </div>
            ...
            <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200">
              ...
              <div className="p-6">
                <p className="text-gray-600 mb-4">
                  Select an assessment type to upload marks for:
                </p>
                <FilteredCurriculum
        ...
        {selectedCurriculumId && (
          <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 mb-10">
            <div className="bg-blue-600 text-white py-3 px-6">
              <h2 className="text-xl font-semibold flex items-center">
                <FaClipboardCheck className="mr-2" /> Student Marks
              </h2>
            </div>
            <div className="p-6">
              <DisplayMarks
        ...
        {/* Termwork Attainment Section */}
        {termworkCurriculum &&
          termworkCurriculum.some((curriculum) => selectedCurriculumId === curriculum.idcurriculum) && (
            <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 mb-10">
              <div className="bg-blue-600 text-white py-3 px-6">
                <h2 className="text-xl font-semibold flex items-center">
                  <FaClipboardCheck className="mr-2" /> Attainment Calculation
                </h2>
              </div>
              <div className="p-6">
                <TermworkAttainmentSaveToDatabase
                  termworkCurriculum={termworkCurriculum}
                  userCourseId={userCourseId}
                />
              </div>
            </div>
